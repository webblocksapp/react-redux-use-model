...
import { mergeQueries, updateQueryListMode } from '@utils';

const entityName = 'TestEntity';
const setLoadMoreMode = (queryKey: string) =>
  updateQueryListMode({ entityName, queryKey }, ListMode.LoadMore);

describe('mergeQueries', () => {
  describe('With standard list mode', () => {
  ...
  describe('With load more list mode', () => {
    it('Merge query with no existing queries.', () => {
      const result = mergeQueries({
        entityName,
        queries: [],
        ...
        queries: [
          {
            queryKey: 'QueryKey',
            ids: ['1', '2', '3', '4', '5'],
            pagination: { page: 0, size: 2, totalElements: 5, totalPages: 3 },
            loading: false,
            listing: false,
            creating: false,
            updating: false,
